...
export default new Vuex.Store({
    state: {
        ...
        missions: [
            {
                id: '0',
                name: 'Попасть в ТОП-10 лучших IT компаний Томска',
                descr: 'Мы хотим стать одной из ведущих компаний Томска с большой узнаваемостью, высококвалифицированными специалистами и хорошей репутацией.  Для этого необходимо повышать качество нашей работы, принимать активное участие в профильных мероприятиях и создать привлекательные условия для работы у нас.'
                ...
                if (goal.id === goalId) {
                    await Vue.axios.get(state.urlBD + 'goals/' + goalId + '/key-results')
                        .then(({data}) => {
                            data.map(kr => {
                                state.people.map(man => {
                                    if (man.id === kr.executor)
                                        kr.executorName = man.name
                                })
                            })
